...
use JdcloudSign\Credentials\CredentialsInterface;
use GuzzleHttp\Psr7;
...
//         var_dump($parsed['headers']['authorization']);
...
//         $parsed['query']['x-jdcloud-expires'] = $this->convertExpires($expiresTimestamp, $startTimestamp);
    ...
    protected function getPayload(RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \RuntimeException('sha256 fail, is not seekable');
        }

        try {
            return Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \RuntimeException('sha256 fail, hash fail');
        }
    }
